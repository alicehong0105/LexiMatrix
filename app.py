...
choice = st.sidebar.radio("系統權限", ["📋 Matrix Core", "🎯 Flash Pulse", "📅 Ebbing Log"])
...
if choice == "📋 Matrix Core":
    ...
    with t_view:
        if not df.empty:
            v_filter = st.radio("Display Protocol", ["Due Today", "All Nodes", "Mastered (L5)"], horizontal=True)
    ...
    with t_edit:
        if not df.empty:
            ...
            with st.form("full_edit_form"):
                ...
                st.write("---")
                e3, e4 = st.columns(2)
                u_syn = e3.text_input("Synonyms", value=row.get('synonyms',''))
                u_coll = e4.text_input("Collocations", value=row.get('collocations',''))
                ...
                if st.form_submit_button("✅ UPDATE MATRIX NODE"):
                    ...
                    st.success("Modified successfully!"); st.rerun()
    ...
    if due:
        ...
        if st.button("EXECUTE"):
            if ans.lower() == q['word'].lower():
                st.success("Synchronized!"); st.balloons()
