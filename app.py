...
if "Matrix Core" in choice:
            ...
            if st.form_submit_button("🚀 SYNC TO CORE"):
                if f_word.strip() and f_mean.strip():
                    ...
                    if is_dup:
                        ...
                        payload = {
                            "word": f_word.strip(), "meaning_zh": f_mean.strip(), "pos": f_pos, # 直接傳列表
    ...
    with t_edit:
        if not df.empty:
            ...
            with st.form("full_edit_all_in"):
                e1, e2 = st.columns(2)
                u_word = e1.text_input("Entry", value=row['word'])
                u_mean = e2.text_input("Definition", value=row['meaning_zh'])
                
                cur_dt = datetime.strptime(str(row['next_review'])[:10], '%Y-%m-%d').date()
                u_date = st.date_input("Manual Next Review", value=cur_dt)
                
                st.write("---")
                ev1, ev2, ev3 = st.columns(3)
                u_v1, u_v2, u_v3 = ev1.text_input("V1", v_list[0]), ev2.text_input("V2", v_list[1]), ev3.text_input("V3", v_list[2])
                
                # 詞性處理
                cur_pos = row.get('pos', [])
                if isinstance(cur_pos, str): cur_pos = cur_pos.split(", ")
                ...
                if st.form_submit_button("✅ UPDATE NODE"):
                    ...
                    if resp.status_code < 300:
                        st.success(f"Synchronized: {u_date}"); st.rerun()
                    else:
