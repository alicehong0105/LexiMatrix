...
st.markdown("""
    <style>
        ...
        .stButton > button { width: 100%; border-radius: 12px; height: 3rem; background: #2d3436; color: white; font-weight: bold; border: none; }
...
if "Matrix Core" in choice:
    ...
    # [C] 修改模式 (修復 Traceback、補齊 Submit Button、補齊全欄位)
    with t_edit:
        if not df.empty:
            ...
            # 劍橋發音連結
            st.link_button(f"🔊 Cambridge: {target_word}", f"https://dictionary.cambridge.org/dictionary/english-chinese-traditional/{target_word.replace(' ', '-')}")
            ...
            with st.form("edit_matrix_form"):
                ...
                if st.form_submit_button("✅ UPDATE MATRIX"):
                    ...
                    if resp.status_code < 300:
                        st.success("Matrix Synchronized!"); st.rerun()
                    else:
    ...
    if not df.empty:
        v_d = st.select_slider("Forecast Dimension (Days)", options=[7, 30, 90, 180, 365], value=30)
